...
@app.before_request
def before():
    if request.path.startswith('/api') and request.path != '/api/token/public':
        ...
        if request.is_json and 'Authorization' in request.headers:  # only JSON requests are allowed
            from boardgameforum.models import Token
    ...
    # boardgameforum wants to be accessed by internet user
...
from boardgameforum import routes
